...
export default function GameScreen({ mode, onGameEnd, onBack }: Props) {
        ...
        if (currentIndex === characters.length - 1) {
          ...
          if (newWordsCompleted >= WORDS_PER_ROUND) {
            ...
            const wpm = Math.round(
              (totalCorrect + 1) / 5 / Math.max(duration, 0.1)
            );
            const accuracy = Math.round(
              ((totalCorrect + 1) / totalChars) * 100
            );
  ...
  const currentWpm = startTime
    ? Math.round(
        totalCorrect / 5 / Math.max((Date.now() - startTime) / 1000 / 60, 0.1)
      )
    : 0;
    ...
    <div className={`game-screen ${showError ? "shake" : ""}`}>
      ...
      <div className="progress-section">
        ...
        <span className="progress-text">
          {wordsCompleted} / {WORDS_PER_ROUND}
        </span>
      </div>
      ...
      <div className="game-main">
        {isPaused ? (
          ...
          <>
            ...
            {mode.id === "beginner" && (
              <KeyboardGuide
                currentKey={characters[currentIndex]?.char || ""}
              />
            )}
          </>
        )}
